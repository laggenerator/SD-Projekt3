...
public:
  ...
  Pair extract_max();
  Pair& find_max();
  Pair* find(const wchar_t* val);
  //z poniższymi mam ten problem, że nie wiem jaki argument maja przyjmowac, rudy nie mowi -- indeks czy wartosc, klucz?
  //delete
  void decrease_key(const wchar_t* val, int i = 1);
  void increase_key(const wchar_t* val, int i = 1);
  void modify_key(const wchar_t* val, int k);
